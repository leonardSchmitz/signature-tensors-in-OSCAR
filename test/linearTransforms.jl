@testset "Equivariance Tests" begin
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    k = 4
    N = 2  # truncation level, number of samples
    T, s = free_trunc_sig_alg_multiv(k, N)

    f_list = [
        s[1,2]*s[1,1] - s[2,2]*s[2,1] + s[3,1]*s[1,2] - s[2,1] + s[4,1] + s[4,2],
    ...
    for f in f_list
        d = 5
        n = 4
        R, a = polynomial_ring_sig_transform(d, n)  # base ring
        TTSn = trunc_tensor_seq(R, k, n)
